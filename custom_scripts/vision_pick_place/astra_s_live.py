...
def main():
    ...
    try:
        while True:
            ...
            # 2026-09-02: removed the red-cube/black-bin detection overlay
            # (per the user's "탐지하는것들을 다 없애줘" during HIL-SERL data
            # collection, where it was just noise) - plain RGB/depth panels
            # now, publishing above is unaffected either way.
            h = min(color.shape[0], depth_vis.shape[0])
            c = cv2.resize(color, (int(color.shape[1] * h / color.shape[0]), h))

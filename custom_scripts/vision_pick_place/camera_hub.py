...
2026-09-02: added CAMERA_HUB_HEADLESS env var (opt-in, default off - same
pattern as astra_s_live.py's ASTRA_LIVE_HEADLESS), per the user closing this
hub's own "RGB | WRIST" window because click_pick_place_zeroshot.py already
folds the published wrist frame into its own window - two windows for the
same wrist feed. When set to "1", skips namedWindow/imshow/waitKey entirely;
capture and /tmp/vsp_*.png publishing (what other scripts actually need) are
unchanged. Stop with Ctrl-C in this mode (no window to press 'q'/ESC in).
"""
...
# 2026-09-01: brightness=0 is now too dark (checked live against a saved
# frame) - room lighting apparently changed since the above. Bumped to 60
# (confirmed visible jaw/cube contrast at that value); re-tune the same way
# if it drifts again.
WRIST_V4L2_CTRLS = {"brightness": 60, "gamma": 1}
...
def main():
    ...
    if headless:
        print("[camera_hub] 실행 중 (headless, CAMERA_HUB_HEADLESS=1) - Ctrl-C로 종료. 프레임을 /tmp/vsp_*.png 로 publish 합니다.")
    else:
    ...
    try:
        while True:
            if headless:
                time.sleep(0.05)  # workers already publish on their own threads - just pace the loop, no window
            else:

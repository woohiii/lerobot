...
int main(int argc, char **argv) try {
    ...
    while (true) {
        ...
        if (colorFrame->format() != OB_FORMAT_RGB) {
            if (colorFrame->format() == OB_FORMAT_MJPG) fmt.setFormatConvertType(FORMAT_MJPG_TO_RGB888);
            else if (colorFrame->format() == OB_FORMAT_UYVY) fmt.setFormatConvertType(FORMAT_UYVY_TO_RGB888);
            else if (colorFrame->format() == OB_FORMAT_YUYV) fmt.setFormatConvertType(FORMAT_YUYV_TO_RGB888);
        ...
        fmt.setFormatConvertType(FORMAT_RGB888_TO_BGR);
        colorFrame = fmt.process(colorFrame)->as<ob::ColorFrame>();
        if (!colorFrame) continue;
